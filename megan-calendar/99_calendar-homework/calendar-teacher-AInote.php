...
負責日曆的外觀設計，包含日曆格子、標題、節日、代辦事項等樣式。
PHP 部分：

取得目前的年月（可由網址參數切換）。
計算本月第一天是星期幾、這個月有幾天。
準備特殊節日與代辦事項資料。
用迴圈建立每一天的資料陣列，包含日期、週次、是否為工作日、節日、代辦事項等。
用 foreach 迴圈將資料印出到 HTML，形成日曆格子。
提供上一月、下一月的切換功能。
萬年曆核心邏輯：

利用 date() 與 strtotime() 處理日期運算。
用陣列儲存每一天的資訊，方便後續擴充（如加上更多事件）。
以迴圈動態產生日曆內容，讓日曆可隨年月變化。 
*/
